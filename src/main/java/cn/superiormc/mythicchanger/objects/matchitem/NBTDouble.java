        ...
        for (String key : tempVal1) {
            ...
            if (parentKeys.length == 3 && nbtItem.hasTag(parentKeys[0], NBTType.NBTTagDouble) && getResult(parentKeys[2], parentKeys[1], parentKeys[0], nbtItem)) {
                ...
                for (String parentKey : parentKeys) {
                    ...
                    } else if (nbtCompound.hasTag(last2Element, NBTType.NBTTagDouble) && getResult(lastElement, last2Element, last3Element, nbtCompound)) {
                        return true;
                    }
                }
                if (nbtCompound != null && nbtCompound.hasTag(last2Element, NBTType.NBTTagDouble) && getResult(lastElement, last2Element, last3Element, nbtCompound)) {
                    return true;
                }
            }
        }
