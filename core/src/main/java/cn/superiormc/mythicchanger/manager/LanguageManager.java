...
public class LanguageManager {
    ...
    public String getPlayerLanguage(Player player) {
        ...
        try {
            if (ConfigManager.configManager.getBoolean("config-files.per-player-language") && !MythicChanger.freeVersion) {
    ...
    private void initLanguages() {
        File langFolder = new File(MythicChanger.instance.getDataFolder(), "languages");
    ...
    private String getMessage(Player player, String key, String... args) {
        ...
        text = text.replace("{plugin_folder}", String.valueOf(MythicChanger.instance.getDataFolder()));
        return text;
    }

    private void saveLanguageFile(String lang, YamlConfiguration config) {
        File file = new File(MythicChanger.instance.getDataFolder(), "languages/" + lang + ".yml");
        try {
            config.save(file);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private YamlConfiguration loadInternalLanguage(String language) {
        ...
        InputStream inputStream = MythicChanger.instance.getResource("languages/" + resourceName + ".yml");
        if (inputStream == null) {
            return null;
        }
